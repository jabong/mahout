 ...
 */
public final class EuclideanDistanceSimilarityDamping extends AbstractSimilarity {

  /**
   * @throws IllegalArgumentException if {@link DataModel} does not have preference values
   */
  public EuclideanDistanceSimilarityDamping(DataModel dataModel) throws TasteException {
   ...
   * @throws IllegalArgumentException if {@link DataModel} does not have preference values
   */
  public EuclideanDistanceSimilarityDamping(DataModel dataModel, Weighting weighting) throws TasteException {
  ...
  double computeResult(int n, double sumXY, double sumX2, double sumY2, double sumXYdiff2) {
	  int k=5;
	  int k1=7;
    return 1.0 / (1.0 + Math.sqrt(sumXYdiff2) / Math.sqrt(n)+Math.max((double)(k-n)/k1, 0));
  }
